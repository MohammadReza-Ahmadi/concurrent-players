...
/**
 * <h3 style="color:#55A3C4"> Two Processes Socket Application that should be called through the jar file </h3>
...
public class TwoProcessSocketCallFromJarApp extends App {
    private static Logger logger = Logger.getLogger(TwoProcessSocketCallFromJarApp.class.getSimpleName());
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            logger.log(Level.FINE, "{0} is started in two processes mode by parameters: port:[{1}], message:[{2}], messageNumber:[{3}] and delayMilliSeconds:[{4}] :::", new Object[]{TwoProcessSocketCallFromJarApp.class.getSimpleName(), port, initiatorMessage, messageNumber, delayMilliSeconds});

            /** create a java process as the initiatorProcess and run the TwoProcessInitiatorSocketRunner into that process */
            initiatorProcess = getPlayerProcess(TwoProcessInitiatorSocketRunner.class, port, initiatorMessage, messageNumber, delayMilliSeconds, jarFileName);
            ...
            receiverProcess = getPlayerProcess(TwoProcessReceiverSocketRunner.class, port, initiatorMessage, messageNumber, delayMilliSeconds, jarFileName);
    ...
    private static Process getPlayerProcess(Class clazz, int port, String message, int messageNumber, int delayMilliSeconds, String jarFileName) throws IOException {
        ...
        jarFileName = (jarFileName == null || jarFileName.isEmpty()) ? JarFromPomConfig.getPropertyValue("structured.two.processes.app").concat(".jar") : jarFileName;
